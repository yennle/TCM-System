...
from tcm.forms import (RegistrationForm, LoginForm, UpdateAccountForm, 
                        PatientForm, UpdatePatientForm, PatientInfoForm, STATE_CHOICES, GENDER_CHOICES
                        ,SymptomForm, GroupForm, SubgroupForm)
...
def load_question_to_database():
    ...
    for symptom in symptoms:
        print(symptom)
        question= PatientQuestion(feature_number=symptom['Feature Number'],feature=symptom['Feature'],question=symptom['English question'],category=symptom['Category'],subgroup=symptom['Subgroup'],group=symptom['Group'], element_number=symptom['Element Number'])
...
def save_test_result(test_id):
    ...
    for result in test_results:
        ...
        target_result = TestResult.query.filter_by(test_id =target_test.id, element_number= result['element_number']).first()
...
def test_patient():
    ...
    patient_questions= PatientQuestion.query.filter(PatientQuestion.category.in_(["P","P/A"]))
...
def createTest(patient_id, questionsList):
    ...
    for question in questionsList:
        answer = TestResult(test_id = test.id, feature_number = question.feature_number, category = question.category, patient= patient_id, element_number = question.element_number)
...
def examine(patient_id):
    ...
    target_test= Test.query.filter_by(patient = patient_id).order_by(Test.test_datetime.desc()).first()
    #print(target_test.id)
    assistant_questions=PatientQuestion.query.filter(PatientQuestion.category.in_(["A"]))
    confirm_questions=PatientQuestion.query.filter(PatientQuestion.category.in_(["P/A"]))
    confirm_results= TestResult.query.filter_by(test_id=target_test.id, category='P/A')
...
def diagnosis(patient_id):
    ...
    patient_questions=PatientQuestion.query.filter(PatientQuestion.category.in_(["P","P/A"]))
    patient_result= TestResult.query.filter(TestResult.category.in_(["P","P/A"]),TestResult.test_id == target_test.id )
    assistant_questions=PatientQuestion.query.filter(PatientQuestion.category.in_(["A"]))
    assistant_result= TestResult.query.filter_by(test_id= target_test.id, category ='A' )
